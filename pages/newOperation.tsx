...
const NewOperation: React.FC = (): JSX.Element => {
  ...
  const { mutate: newOperation, isLoading } = useNewOperation(
    (data: ResponseType | null) => setResponse(data),
    (error: AxiosError | null) => setError(error),
  );
  const [operation, setOperation] = useState<string>();
  const [fields, setFields] = useState<
    Record<string, OperationFields> | undefined
  >();
  const { data: operations = [] } = data;
  const handleSubmit = (e: SyntheticEvent) => {
    e.preventDefault();
    const operationObj = operations.find(
      (op: Operation) => op.type === operation,
    ) as Operation;
    const newFields: Record<string, string | number> = fields
      ? Object.fromEntries(
          Object.entries(fields).map(
            ([key, value]: [string, OperationFields]) => [key, value.value],
          ),
        )
      : {};
    newOperation({
      ...
      <Form onSubmit={handleSubmit}>
        <Form.Field required={true}>
          ...
          <Dropdown
            ...
            required
            selection
            options={operations.map((op: Operation) => ({
              key: op.id,
              value: op.type,
              text: `${op.type} (cost: ${op.cost})`,
            }))}
            value={operation}
        ...
        {fields &&
          Object.entries(fields).map(
            ([key, value]: [string, OperationFields]) => (
        ...
        <Button type="submit" disabled={!operation} loading={isLoading}>
          Calculate
        </Button>
      </Form>
      {response && (
